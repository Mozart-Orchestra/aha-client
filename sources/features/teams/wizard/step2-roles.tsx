...
import { RoleMarketModal } from './RoleMarketModal';
...
export const Step2Roles = React.memo(function Step2Roles({ onNext, onBack }: Step2RolesProps) {
    ...
    const removeRole = React.useCallback((roleId: string) => {
        setRoles((prev) => prev.filter((role) => role.id !== roleId));
        setExpandedRoleId((prev) => (prev === roleId ? null : prev));
        setAdvancedRoleId((prev) => (prev === roleId ? null : prev));
    }, []);
    ...
    const handleAddRoleFromMarket = React.useCallback(() => {
        Modal.show({
            component: RoleMarketModal,
            props: {
                existingRoleIds: roles.map((role) => role.roleId),
                defaultMachineId: firstMachineId,
                goal: state.goal,
                context: [state.teamName, state.workingDirectory].filter(Boolean).join(' · '),
                onSelectRole: (newRole: RoleConfig) => {
    ...
    return (
        ...
        >
                                ...
                                <View style={styles.expandedBody}>
                                    ...
                                    {isAdvancedOpen && (
                                        <View style={styles.advancedCard}>
                                            <Text style={styles.rowLabel}>Role Root Path</Text>
                                            <TextInput
                                                style={[
                                                ...
                                                value={role.rootPath || ''}
                                                onChangeText={(rootPath) =>
                                                    updateRole(role.id, (item) => ({
                                                        ...item,
                                                        rootPath,
                                                    }))
                                                }
                                                placeholder="Optional override for this role"
                                                placeholderTextColor={theme.colors.input.placeholder}
                                                autoCapitalize="none"
                                                autoCorrect={false}
                                            />
                                            <Text style={styles.advancedHint}>
                                                Model, skills, MCP, and plugins stay configurable after creation.
                                            </Text>
                                        </View>
                                    )}
                                </View>
            ...
            <View style={styles.footer}>
                ...
                <Pressable style={[styles.nextButton, roles.length === 0 && styles.nextButtonDisabled]} onPress={handleNext}>
