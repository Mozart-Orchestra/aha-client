...
Evolution summary surfaces should converge through `sources/hooks/useEvolutionSummary.ts` and `sources/components/EvolutionSummaryCard.tsx`; treat `/v1/teams/:teamId/evolution/summary` as the canonical team-evolution digest, and keep morning-briefing variants as compact entry points that link into `/web/team-info` or `teams/[id]`.
...
Task actions on default web surfaces should converge through `sources/sync/apiTasks.ts` + `sources/hooks/useTeamTasks.ts` + `sources/components/TaskDetailModal.tsx`; keep chat task cards as entry points and `/web/board` as the primary edit/review/gantt surface.
